...
export function buildInvoicePrintDocument(payload: InvoicePrintPayload, format: "a4" | "receipt" = "a4") {
  ...
  const measurementsSection = measurements && measurementCells.length ? `<fieldset class="measurements"><legend>${label("Optional")} · ${label("Enabled from tailoring order")}</legend><p class="measurements-title">${label("Customer Measurements")} — ${label(`Version ${measurements.version}`)}</p><p class="measurements-subtitle">${label('Included because "Include measurements on invoice" was checked for this order')}</p><div class="measurement-grid">${measurementCells.map(cell => `<div class="measurement-cell"><span class="measurement-label">${escapeHtml(cell.label)}</span><span class="measurement-value">${escapeHtml(cell.value)}</span></div>`).join("")}</div></fieldset>` : "";
  return `<!doctype html><html lang="${language}" dir="${isArabic ? "rtl" : "ltr"}"><head><meta charset="utf-8" /><meta name="viewport" content="width=device-width, initial-scale=1" /><title>${escapeHtml(payload.invoice.invoiceNumber)}</title><style>
    ...
    * { box-sizing: border-box; }
    ...
    .measurements { margin: 26px 0 0; padding: 4px 20px 18px; border: 1.5px dashed #c8973f; border-radius: 12px; } .measurements legend { padding: 0 8px; color: #a8751f; font-size: 10px; font-weight: 700; letter-spacing: .1em; text-transform: uppercase; } .measurements-title { margin: 2px 0 0; color: #1f496d; font-size: 15px; font-weight: 700; } .measurements-subtitle { margin: 4px 0 0; color: #64748b; font-size: 11px; } .measurement-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px 32px; margin-top: 16px; } .measurement-cell { font-size: 11px; } .measurement-label { display: block; color: #64748b; font-size: 9px; font-weight: 700; letter-spacing: .06em; text-transform: uppercase; } .measurement-value { display: block; margin-top: 3px; font-weight: 700; color: #17253a; font-size: 13px; }
